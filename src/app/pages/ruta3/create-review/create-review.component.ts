...
import { IBookForReview } from '../../../models/books.model';
...
export class CreateReviewComponent implements OnInit {
  ...
  books: IBookForReview[] = [];
  selectedBook?: IBookForReview | null;
  ...
  ngOnInit() {
    this.bookService.GetAllBooks().subscribe((books) => {
      this.books = books.map(b => ({
        id: b.id,
        title: b.title,
        authorName: 'Autor X',       // Acá podés usar datos reales si los tenés
        publisherName: 'Editorial Y' // Lo mismo para editorial
      }));
    });
  }
  ...
  guardar(data: { comment: string; rating: number }) {
    ...
    this.reviewService.crear(reviewPayload).subscribe({
      ...
      error: () => {
        alert('Ocurrió un error al crear la reseña. Intentá nuevamente.');
      }
    });
  }
}
